...
app.get('/api/utilisateurs/search', authMiddleware, async function(req, res) {
  try { var q = (req.query.q || '').trim(); if (q.length < 1) return res.json({ success: true, data: [] });
    var [r] = await db.query('SELECT ID, CODEUTILISATEUR, UTILISATEUR, SUPERVISEUR, TYPEUTILISATEUR, RESEAU FROM utilisateur WHERE TYPEUTILISATEUR = ? AND (UTILISATEUR LIKE ? OR SUPERVISEUR LIKE ? OR CAST(CODEUTILISATEUR AS CHAR) LIKE ?) ORDER BY UTILISATEUR LIMIT 30', ['dm', '%'+q+'%', '%'+q+'%', '%'+q+'%']); res.json({ success: true, data: r }); }
  catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
