...
async function applyMonthRules(partenariatId, months) {
  ...
  for (var i = 0; i < months.length; i++) {
    ...
    if (s.manual) {
      // ═══ Mois MANUEL (admin) ═══
      // Il a été ouvert par l'admin, il reste actif jusqu'à clôture manuelle
      // Ne pas y toucher s'il est actif ou déjà closed
      if (s.status === 'active' || s.status === 'closed') continue;
      // S'il est inactive et qu'aucun mois n'est actif et qu'il est activable → l'activer
      if (s.status === 'inactive' && !activeMonth && isMonthActivatable(ym)) {
...
app.post('/api/partenariats/:id/close-month', authMiddleware, async function(req, res) {
  try {
    ...
    // Activer le prochain mois marqué "manuel" (autoActivate = manual_close = 1)
    var [all] = await db.query('SELECT month, manual_close FROM partenariat_month_status WHERE partenariat_id = ? ORDER BY month', [id]);
    for (var i = 0; i < all.length; i++) {
      if (all[i].month > month && all[i].manual_close) {
        await db.query('UPDATE partenariat_month_status SET status = ? WHERE partenariat_id = ?', ['inactive', id]);
        await db.query('UPDATE partenariat_month_status SET status = ? WHERE partenariat_id = ? AND month = ?', ['active', id, all[i].month]);
        break;
      }
    }
    res.json({ success: true });
