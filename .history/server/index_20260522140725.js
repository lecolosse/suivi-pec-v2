...
app.post('/api/partenariats/:id/close-month', authMiddleware, async function(req, res) {
  try {
    ...
    for (var i = 0; i < allMonths.length; i++) {
      var m = allMonths[i];
      console.log('Check:', m.month, 'manual_close:', m.manual_close, 'canActivate:', canActivateMonth(m.month), '>month:', m.month > month);
      if (m.month > month && m.manual_close && canActivateMonth(m.month)) {
        console.log('ACTIVATING:', m.month);
        await db.query('UPDATE partenariat_month_status SET status = ? WHERE partenariat_id = ?', ['inactive', id]);
        await db.query('UPDATE partenariat_month_status SET status = ? WHERE partenariat_id = ? AND month = ?', ['active', id, m.month]);
        break;
      }
    }
    console.log('--- end close-month ---');
