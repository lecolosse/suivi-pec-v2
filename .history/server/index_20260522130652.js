...
//################## SIMULATION DATE ##################
const mockedDate = new Date('2025-06-01T22:00:00Z');
...
// ── Statut des mois ──
// 'inactive' = pas encore commencé / en attente
// 'active'   = le mois en cours de saisie (UN SEUL à la fois)
// 'closed'   = clôturé (manuellement ou automatiquement si autoClose=true)
...
async function computeMonthStatuses(partenariatId, months) {
  ...
  // Si aucun statut en base, initialiser tout à 'inactive'
  ...
  // Vérifier les clôtures automatiques (pour les mois autoClose=false, on les laisse actifs indéfiniment)
  var now = new Date();
  for (var i = 0; i < months.length; i++) {
    var ym = months[i];
    var entry = dbStatuses[ym];
    if (!entry || entry.manualClose) continue; // les mois manuels ne se clôturent pas auto

    if (entry.status === 'active') {
      // Vérifier si la date de clôture auto est dépassée (45 jours après activation = 15 du mois M+2)
      var parts = ym.split('-');
      var y = parseInt(parts[0]), m = parseInt(parts[1]);
      var closeM = m + 2, closeY = y;
      if (closeM > 12) { closeM -= 12; closeY++; }
      var autoCloseDate = new Date(closeY, closeM - 1, 15, 23, 59, 59);
    ...
    if (entry.status === 'inactive') {
      // Vérifier si la date d'activation auto est arrivée (1er du mois suivant)
      var parts2 = ym.split('-');
      var y2 = parseInt(parts2[0]), m2 = parseInt(parts2[1]);
      var actM = m2 + 1, actY = y2;
      if (actM > 12) { actM = 1; actY++; }
      var autoActDate = new Date(actY, actM - 1, 1, 0, 0, 0);
      if (now >= autoActDate) {
        // Vérifier qu'aucun autre mois n'est actif
        var hasActive = false;
        for (var m2 in dbStatuses) {
          if (dbStatuses[m2].status === 'active') { hasActive = true; break; }
        }
        if (!hasActive) {
  ...
  // Ajouter aussi manualClose dans la réponse
  result._manual = {};
  for (var m in dbStatuses) {
    result._manual[m] = dbStatuses[m].manualClose;
  }
...
function buildPartnership(p, prods, pharmas, qtys, creatorName, monthStatuses) {
  ...
  var monthStatus = {};
  for (var m in monthStatuses) {
    if (m === '_manual') continue;
    monthStatus[m] = monthStatuses[m];
  }
...
function generateMonthsFromDB(startMonth, endMonth) {
  ...
  var cY = p1[0], cM = p1[1];
  while (cY < p2[0] || (cY === p2[0] && cM <= p2[1])) {
    months.push(cY + '-' + String(cM).padStart(2, '0')); cM++; if (cM > 12) { cM = 1; cY++; }
  }
  return months;
}
...
async function loadRelated(pId) {
  ...
  var months = [];
  if (pRow.length > 0) { months = generateMonthsFromDB(pRow[0].start_month, pRow[0].follow_up_end_month || pRow[0].end_month); }
...
// PUT: Configurer un mois (admin) — active/désactive + autoActivate switch
app.put('/api/partenariats/:id/month-config', authMiddleware, async function(req, res) {
  if (req.user.type !== 'admin') return res.status(403).json({ success: false, error: 'Admin seulement' });
  try {
    var id = req.params.id;
    var month = req.body.month;
    var newStatus = req.body.status;     // 'active' ou 'inactive' ou 'pending'
    var autoActivate = req.body.autoActivate ? 1 : 0;  // si ON, s'active automatiquement après clôture du mois précédent
...
// POST: Clôturer le mois actif — active automatiquement le mois suivant si autoActivate=true
app.post('/api/partenariats/:id/close-month', authMiddleware, async function(req, res) {
  try {
    ...
    // Trouver le mois suivant avec autoActivate=true et l'activer
    ...
    for (var i = 0; i < allMonths.length; i++) {
      if (allMonths[i].month > month && allMonths[i].manual_close) {
...
// POST: Clôturer le mois actif (dm ou admin)
app.post('/api/partenariats/:id/close-month', authMiddleware, async function(req, res) {
  try {
    var id = req.params.id, month = req.body.month;
    if (req.user.type === 'dm') {
