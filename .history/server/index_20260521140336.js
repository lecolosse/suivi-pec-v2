...
// PUT: Configurer un mois (admin) — active/désactive + manualClose switch
app.put('/api/partenariats/:id/month-config', authMiddleware, async function(req, res) {
  if (req.user.type !== 'admin') return res.status(403).json({ success: false, error: 'Admin seulement' });
  try {
    var id = req.params.id;
    var month = req.body.month;
    var newStatus = req.body.status;     // 'active' ou 'inactive'
    var manualClose = req.body.manualClose ? 1 : 0;

    // Si on active ce mois, désactiver tous les autres
    if (newStatus === 'active') {
      await db.query('UPDATE partenariat_month_status SET status = ? WHERE partenariat_id = ?', ['inactive', id]);
    }

    await db.query('INSERT INTO partenariat_month_status (partenariat_id, month, status, manual_close) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE status = VALUES(status), manual_close = VALUES(manual_close)', [id, month, newStatus, manualClose]);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
