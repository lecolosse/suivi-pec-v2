...
// ── Logique d'auto-activation d'UN SEUL mois à la fois ──

function getMonthStatus(ym) {
  // Retourne: 'closed' (trop tard), 'active' (mois en cours), 'pending' (pas encore activé)
  var now = new Date();
  var parts = ym.split('-');
  var year = parseInt(parts[0]);
  var month = parseInt(parts[1]);

  // Activation: 1er du mois suivant
  var activationMonth = month + 1;
  var activationYear = year;
  if (activationMonth > 12) { activationMonth = 1; activationYear++; }
  var activationDate = new Date(activationYear, activationMonth - 1, 1, 0, 0, 0);

  // Clôture: 15 du mois M+2
  var closeMonth = month + 2;
  var closeYear = year;
  if (closeMonth > 12) { closeMonth -= 12; closeYear++; }
  var closeDate = new Date(closeYear, closeMonth - 1, 15, 23, 59, 59);

  if (now < activationDate) return 'pending';   // pas encore activé
  if (now > closeDate) return 'closed';           // clôturé automatiquement
  return 'active';                                 // en cours
}

async function computeMonthStatuses(partenariatId, months) {
  // Récupérer les statuts déjà en base
  var [mss] = await db.query('SELECT month, status FROM partenariat_month_status WHERE partenariat_id = ?', [partenariatId]);
  var dbStatuses = {};
  for (var i = 0; i < mss.length; i++) {
    dbStatuses[mss[i].month] = mss[i].status;
  }

  // Trouver s'il y a déjà un mois marqué comme 'active' en base (clôture manuelle pas encore faite)
  var hasActive = false;
  for (var m in dbStatuses) {
    if (dbStatuses[m] === 'active') { hasActive = true; break; }
  }

  // Calculer le statut automatique pour chaque mois
  for (var i = 0; i < months.length; i++) {
    var ym = months[i];
    var autoStatus = getMonthStatus(ym);
    var dbStatus = dbStatuses[ym];

    // Si l'admin a forcé un statut, on le respecte
    if (dbStatus === 'admin_active') {
      dbStatuses[ym] = 'active'; continue;
    }
    if (dbStatus === 'admin_closed') {
      dbStatuses[ym] = 'closed'; continue;
    }

    // Si le mois est dans le passé lointain → closed (auto)
    if (autoStatus === 'closed') {
      if (dbStatus !== 'closed') {
        await db.query('INSERT INTO partenariat_month_status (partenariat_id, month, status) VALUES (?,?,?) ON DUPLICATE KEY UPDATE status=VALUES(status)', [partenariatId, ym, 'closed']);
      }
      dbStatuses[ym] = 'closed';
      continue;
    }

    // Si le mois n'est pas encore activable → pending
    if (autoStatus === 'pending') {
      if (!dbStatus || dbStatus === 'pending') {
        await db.query('INSERT INTO partenariat_month_status (partenariat_id, month, status) VALUES (?,?,?) ON DUPLICATE KEY UPDATE status=VALUES(status)', [partenariatId, ym, 'pending']);
      }
      dbStatuses[ym] = 'pending';
      continue;
    }

    // Si le mois est dans sa période d'activité ET qu'aucun autre mois n'est actif → active
    if (autoStatus === 'active' && !hasActive) {
      if (dbStatus !== 'active') {
        await db.query('INSERT INTO partenariat_month_status (partenariat_id, month, status) VALUES (?,?,?) ON DUPLICATE KEY UPDATE status=VALUES(status)', [partenariatId, ym, 'active']);
      }
      dbStatuses[ym] = 'active';
      hasActive = true; // UN SEUL mois actif
      continue;
    }

    // Mois dans sa période mais un autre mois est déjà actif → pending (en attente que l'actif soit clôturé)
    if (autoStatus === 'active' && hasActive) {
      dbStatuses[ym] = 'pending';
      await db.query('INSERT INTO partenariat_month_status (partenariat_id, month, status) VALUES (?,?,?) ON DUPLICATE KEY UPDATE status=VALUES(status)', [partenariatId, ym, 'pending']);
    }
...
function buildPartnership(p, prods, pharmas, qtys, creatorName, monthStatuses) {
  ...
  var monthStatus = {};
  for (var m in monthStatuses) { monthStatus[m] = monthStatuses[m]; }
...
function generateMonthsFromDB(startMonth, endMonth) {
  ...
  var parts1 = startMonth.split('-').map(Number);
  var parts2 = endMonth.split('-').map(Number);
  var cY = parts1[0], cM = parts1[1];
  while (cY < parts2[0] || (cY === parts2[0] && cM <= parts2[1])) {
...
async function loadRelated(pId) {
  ...
  var months = [];
  if (pRow.length > 0) {
    var endM = pRow[0].follow_up_end_month || pRow[0].end_month;
    months = generateMonthsFromDB(pRow[0].start_month, endM);
  }
...
app.get('/api/partenariats', authMiddleware, async function(req, res) {
  try {
    ...
    else if (role === 'sup') { [rows] = await db.query('SELECT DISTINCT p.* FROM partenariats p JOIN users u ON p.created_by = u.ID JOIN utilisateur ut ON u.login = ut.UTILISATEUR WHERE ut.SUPERVISEUR = ? ORDER BY p.created_at DESC', [req.user.login]); }
    else { return res.json({ success: true, data: [] }); }

    var result = [];
    for (var i = 0; i < rows.length; i++) {
      var p = rows[i]; var rel = await loadRelated(p.id); var creatorName = await getCreatorName(p.created_by);
...
app.get('/api/partenariats/:id', authMiddleware, async function(req, res) {
  try {
    ...
    if (role === 'sup') { var [chk] = await db.query('SELECT 1 FROM partenariats p JOIN users u ON p.created_by = u.ID JOIN utilisateur ut ON u.login = ut.UTILISATEUR WHERE p.id = ? AND ut.SUPERVISEUR = ?', [req.params.id, req.user.login]); if (chk.length === 0) return res.status(403).json({ success: false, error: 'Accès refusé' }); }
    var rel = await loadRelated(p.id); var creatorName = await getCreatorName(p.created_by);
...
app.put('/api/partenariats/:id/quantity', authMiddleware, async function(req, res) {
  try {
    ...
    await db.query('INSERT INTO partenariat_quantities (partenariat_id, pharmacy_name, product_name, month, quantity) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE quantity = VALUES(quantity)', [id, req.body.pharmacyName, req.body.productName, req.body.month, req.body.quantity || 0]);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
...
// POST: Activer manuellement (admin) — permet d'activer les mois passés
app.post('/api/partenariats/:id/activate-month', authMiddleware, async function(req, res) {
  if (req.user.type !== 'admin') return res.status(403).json({ success: false, error: 'Admin seulement' });
  try {
    // Désactiver tout autre mois actif
    await db.query('UPDATE partenariat_month_status SET status = ? WHERE partenariat_id = ? AND status = ?', ['pending', req.params.id, 'active']);
    // Activer le mois demandé
    await db.query('INSERT INTO partenariat_month_status (partenariat_id, month, status) VALUES (?,?,?) ON DUPLICATE KEY UPDATE status = VALUES(status)', [req.params.id, req.body.month, 'active']);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});

// POST: Clôturer le mois actif (dm ou admin) — débloque le mois suivant automatiquement
app.post('/api/partenariats/:id/close-month', authMiddleware, async function(req, res) {
  try {
    ...
    // Le mois suivant sera automatiquement activé par computeMonthStatuses au prochain GET
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
