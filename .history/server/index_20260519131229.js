...
// ════════════════════════════════════════════════════════════
// REFERENCES  –  recherche uniquement par chiffres
// ════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════
// PROSPECTS  –  recherche dans la table prospect
// ════════════════════════════════════════════════════════════
app.get('/api/prospects', async (_req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT ID, CODEPROSPECT, NOMPRENOM, SPECIALITE, SECTEUR
       FROM prospect
       ORDER BY NOMPRENOM`,
    );
    res.json({ success: true, data: rows });
...
app.get('/api/prospects/search', async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT ID, CODEPROSPECT, NOMPRENOM, SPECIALITE, SECTEUR
       FROM prospect
       WHERE NOMPRENOM LIKE ? OR SPECIALITE LIKE ? OR SECTEUR LIKE ? OR CAST(CODEPROSPECT AS CHAR) LIKE ?
       ORDER BY NOMPRENOM
       LIMIT 30`,
      [`%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`],
    );
...
// ════════════════════════════════════════════════════════════
// UTILISATEURS  –  liste depuis la table utilisateur (délégués)
// ════════════════════════════════════════════════════════════
app.get('/api/utilisateurs', async (_req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT ID, CODEUTILISATEUR, UTILISATEUR, SUPERVISEUR, TYPEUTILISATEUR, RESEAU
       FROM utilisateur
...
app.get('/api/utilisateurs/search', async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT ID, CODEUTILISATEUR, UTILISATEUR, SUPERVISEUR, TYPEUTILISATEUR, RESEAU
       FROM utilisateur
       WHERE UTILISATEUR LIKE ? OR SUPERVISEUR LIKE ? OR CAST(CODEUTILISATEUR AS CHAR) LIKE ?
       ORDER BY UTILISATEUR
       LIMIT 30`,
      [`%${q}%`, `%${q}%`, `%${q}%`],
    );
    ...
    console.error('Utilisateur search error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
app.get('/api/pharmacies', async (_req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT ID as ID_PHARMACIE, NOMPRENOM as NomPharmacie, SPECIALITE as SpecialitePharmacie, SECTEUR as SecteurPharmacie FROM prospect ORDER BY NOMPRENOM',
    );
    res.json({ success: true, data: rows });
  } catch (error) {
  ...
  try {
    ...
    const [result] = await conn.query(
      `INSERT INTO partenariats
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        b.year, b.reference, b.typeManifestation, b.manifestation || null,
        b.startMonth, b.endMonth, b.followUpEndMonth || null,
        b.prospect || null, b.supervisor || null, b.delegates || null,
        b.budget || 0, b.potentialPharmacies || 0,
      ],
    );
    ...
    if (b.products && b.products.length > 0) {
      for (const prod of b.products) {
        ...
        products.push({ id: String(r.insertId), name: prod.name, boxes: prod.boxes || 0 });
      }
    }
    ...
    if (b.pharmacies && b.pharmacies.length > 0) {
      for (const phar of b.pharmacies) {
        ...
        pharmacies.push({ id: String(r.insertId), name: phar.name });
      }
    }
    ...
    res.json({
      ...
      data: {
        ...
        year: b.year, reference: b.reference,
        typeManifestation: b.typeManifestation,
        manifestation: b.manifestation || '',
        startMonth: b.startMonth, endMonth: b.endMonth,
        followUpEndMonth: b.followUpEndMonth || '',
        prospect: b.prospect || '', supervisor: b.supervisor || '',
        delegates: b.delegates || '',
        budget: b.budget || 0, potentialPharmacies: b.potentialPharmacies || 0,
        products, pharmacies, tableData: {},
