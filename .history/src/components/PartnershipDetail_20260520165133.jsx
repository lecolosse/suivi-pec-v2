...
export default function PartnershipDetail({ partnership, onUpdate, onDelete, onBack, userRole }) {
  ...
  function getEditableMonths() {
    ...
    for (var i = 0; i < months.length; i++) {
      ...
      var editable = isAdmin || (s === 'active' && isDm);
      r[m] = { status: s, editable: editable };
    }
    return r;
  }
  var editableMonths = getEditableMonths();

  function canActivate(month) { return isAdmin && (monthStatus[month]||'locked') !== 'active'; }
  function canClose(month) { return (isAdmin || isDm) && (monthStatus[month]||'locked') === 'active'; }
  function canUnlock(month) { return isAdmin && (monthStatus[month]||'locked') === 'closed'; }

  async function act(m) { try{await api.activateMonth(partnership.id,m);refresh();}catch(e){console.error(e);} }
  async function close(m) { try{await api.closeMonth(partnership.id,m);refresh();}catch(e){console.error(e);} }
  async function unlock(m) { try{await api.unlockMonth(partnership.id,m);refresh();}catch(e){console.error(e);} }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* STATUTS DES MOIS */}
      {months.length>0 && (
        <div className="flex flex-wrap items-center gap-3" style={{background:isAdmin?'#fefce8':'#dcfce7',border:'1px solid '+(isAdmin?'#fde68a':'#86efac'),borderRadius:12,padding:'12px 16px',fontSize:12}}>
          <span style={{fontWeight:700,color:isAdmin?'#854d0e':'#16a34a'}}>{isAdmin?'🔧 Gestion des mois :':'📅 Statut des mois :'}</span>
          {months.map(function(m){
            var s = monthStatus[m] || 'locked';
            var label = s==='active'?'Actif':s==='closed'?'Clôturé':'En attente';
            var bg = s==='active'?'#dcfce7':s==='closed'?'#f1f5f9':'#fef3c7';
            var clr = s==='active'?'#16a34a':s==='closed'?'#64748b':'#b45309';
            return (
              <span key={'gm-'+m} style={{display:'flex',alignItems:'center',gap:6,background:'#fff',borderRadius:8,padding:'6px 10px',border:'1px solid #e2e8f0'}}>
                ...
                <span style={{fontSize:10,fontWeight:500,padding:'2px 6px',borderRadius:4,background:bg,color:clr}}>{label}</span>
                {canActivate(m) && <button onClick={function(){act(m);}} style={{fontSize:10,padding:'3px 8px',borderRadius:4,border:'1px solid #16a34a',background:'#dcfce7',color:'#16a34a',cursor:'pointer',fontWeight:600}}>Activer</button>}
                {canClose(m) && <button onClick={function(){close(m);}} style={{fontSize:10,padding:'3px 8px',borderRadius:4,border:'1px solid #f59e0b',background:'#fef3c7',color:'#b45309',cursor:'pointer',fontWeight:600}}>Clôturer</button>}
                {canUnlock(m) && <button onClick={function(){unlock(m);}} style={{fontSize:10,padding:'3px 8px',borderRadius:4,border:'1px solid #6366f1',background:'#eef2ff',color:'#4f46e5',cursor:'pointer',fontWeight:600}}>Débloquer</button>}
              </span>
            );
          })}
        </div>
      ...
      {isDm && activeMonth && (
        <div className="flex items-center justify-between" style={{background:'#dcfce7',border:'1px solid #86efac',borderRadius:12,padding:'10px 16px',fontSize:12}}>
          ...
          <button onClick={function(){close(activeMonth);}} style={{fontSize:11,fontWeight:600,padding:'5px 12px',borderRadius:6,border:'1px solid #f59e0b',background:'#fef3c7',color:'#b45309',cursor:'pointer'}}>Clôturer ce mois</button>
        </div>
      )}
      ...
      {!canEdit && !isDm && <div style={{background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:12,padding:'12px 16px',fontSize:13,fontWeight:500,color:'#1d4ed8'}}>🔒 Lecture seule</div>}
          ...
          <div style={{overflow:'auto',maxHeight:'45vh'}}>
            <table style={{borderCollapse:'collapse',width:'100%',fontSize:12}}>
              <thead><tr style={{background:'#4f46e5'}}>
                ...
                {months.map(function(m){var s=monthStatus[m]||'locked';return <th key={m} style={thStyle({background:s==='active'?'#16a34a':s==='closed'?'#94a3b8':'#4f46e5'})}>{formatMonth(m)}{s==='active'?' ●':' 🔒'}</th>;})}
              ...
              <tbody>
                {pharmacies.map(function(ph,phi){var rs=products.length+1;var bg=phi%2===0?'#fff':'#f8fafc';return <Fragment key={ph.id}>
                  {products.map(function(prd,pri){var t=pt(ph.id,prd.name);return <tr key={ph.id+'-'+prd.id} style={{background:bg}}>
                    ...
                    {months.map(function(mo){var v=cell(ph.id,prd.name,mo);var em=editableMonths[mo];var editable=em?em.editable:false;var mbg=(monthStatus[mo]||'locked')==='active'?'#f0fdf4':'#fff';return <td key={mo} style={{border:'1px solid #e2e8f0',padding:0,textAlign:'center',background:mbg}}>{editable?<input type="number" min="0" value={v||''} onChange={function(e){upd(ph.id,phN(ph.id),prd.name,mo,e.target.value);}} placeholder="0" style={{width:'100%',textAlign:'center',border:'none',background:'transparent',outline:'none',fontSize:12,padding:'6px 2px',boxSizing:'border-box'}} />:<span style={{fontSize:12,color:'#94a3b8',padding:'6px 2px',display:'block'}}>{v||''}</span>}</td>;})}
