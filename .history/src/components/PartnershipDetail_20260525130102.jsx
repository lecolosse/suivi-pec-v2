...
export default function PartnershipDetail({ partnership, onUpdate, onDelete, onBack, userRole }) {
  ...
  var canEdit = (userRole === 'admin' || userRole === 'dm') && !partnership.contratStatus;
  ...
  async function configMonth(month, status, autoActivate) {
    try {
      await api.configureMonth(partnership.id, month, status, autoActivate);
      refresh();
    } catch (e) { console.error(e); }
  }

  var [closingMonth, setClosingMonth] = useState(false);

  async function closeMonth(month) {
    setClosingMonth(true);
    try {
      await api.closeMonth(partnership.id, month);
  ...
  async function refresh() {
    try {
      var p = await api.getPartenariat(partnership.id);
      onUpdate(p);
    } catch (e) { console.error(e); }
  }
    ...
    <div className="space-y-4">
      ...
      {isAdmin && months.length > 0 && (
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', padding: '16px 20px' }}>
          ...
          <p style={{ fontSize: 11, color: '#94a3b8', marginBottom: 14 }}>
            ⬤ Mois de saisie en cours · 🔘 Activation automatique après clôture du mois précédent
          </p>
              ...
              return (
                <div key={'cfg-' + m} style={{
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', flex: 1 }}>
                    ...
                    {isActive && <span style={{ fontSize: 10, background: '#16a34a', color: '#fff', padding: '2px 8px', borderRadius: 4, fontWeight: 600 }}>EN COURS</span>}
                  ...
                  {/* 🔘 Switch : activation automatique */}
                  <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', flexShrink: 0 }} title={mc ? "S'activera automatiquement après clôture du mois précédent" : "Activable uniquement manuellement par l'admin"}>
                    <span style={{ fontSize: 11, color: '#64748b' }}>{mc ? 'Auto' : 'Admin'}</span>
                    <div
      ...
      {isDm && activeMonth && (
        <div className="flex items-center justify-between" style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: 12, padding: '10px 16px', fontSize: 12 }}>
          ...
          <button onClick={function () { closeMonth(activeMonth); }} disabled={closingMonth} style={{ fontSize: 11, fontWeight: 600, padding: '5px 12px', borderRadius: 6, border: '1px solid #f59e0b', background: '#fef3c7', color: '#b45309', cursor: closingMonth ? 'not-allowed' : 'pointer', opacity: closingMonth ? 0.6 : 1 }}>{closingMonth ? '⏳ Clôture...' : 'Clôturer ce mois'}</button>
        </div>
      )}
      {isDm && !activeMonth && !partnership.contratStatus && <div style={{ background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 12, padding: '12px 16px', fontSize: 13, color: '#64748b' }}>⏳ Aucun mois ouvert pour la saisie</div>}

      {partnership.contratStatus && (
        <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 24 }}>🔒</span>
          <div>
            <p style={{ fontWeight: 700, fontSize: 14, color: '#dc2626' }}>
              Contrat clôturé — {partnership.contratStatus === 'Honnore' ? '✅ A honoré le contrat' : partnership.contratStatus === 'NonHonnore' ? '❌ N\'a pas honoré le contrat' : partnership.contratStatus === 'Desistement' ? '🚫 Désistement' : '📝 ' + partnership.contratStatus}
            </p>
            <p style={{ fontSize: 12, color: '#64748b' }}>Clôturé par {partnership.contratClosedBy} le {partnership.contratClosedAt ? new Date(partnership.contratClosedAt).toLocaleDateString('fr-FR') : ''}. Aucune modification possible.</p>
            {partnership.contratComment && <p style={{ fontSize: 12, color: '#475569', marginTop: 4, fontStyle: 'italic' }}>"{partnership.contratComment}"</p>}
          </div>
        </div>
      )}
      ...
      {isSup && (
        <div>
          <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 12, padding: '12px 16px', fontSize: 13, fontWeight: 500, color: '#1d4ed8' }}>🔒 Consultation uniquement</div>
          {!partnership.contratStatus && <CloseContractModal partnership={partnership} onCloseContract={async function (status, comment) { try { await api.closeContract(partnership.id, status, comment); refresh(); } catch (e) { console.error(e); } }} />}
          {partnership.contratStatus && (
            <div style={{ marginTop: 8, background: partnership.contratStatus === 'Honnore' ? '#f0fdf4' : '#fef2f2', border: '1px solid ' + (partnership.contratStatus === 'Honnore' ? '#86efac' : '#fecaca'), borderRadius: 12, padding: '12px 16px', fontSize: 12 }}>
              <span style={{ fontWeight: 700, color: partnership.contratStatus === 'Honnore' ? '#16a34a' : '#dc2626' }}>📋 Contrat clôturé : {partnership.contratStatus === 'Honnore' ? '✅ A honoré le contrat' : partnership.contratStatus === 'NonHonnore' ? '❌ N\'a pas honoré le contrat' : partnership.contratStatus === 'Desistement' ? '🚫 Désistement' : partnership.contratStatus}</span>
              <span style={{ marginLeft: 12, color: '#64748b' }}>par {partnership.contratClosedBy} le {partnership.contratClosedAt ? new Date(partnership.contratClosedAt).toLocaleDateString('fr-FR') : ''}</span>
              {partnership.contratComment && <div style={{ marginTop: 4, color: '#475569', fontStyle: 'italic' }}>"{partnership.contratComment}"</div>}
            </div>
          )}
        </div>
      )}
            ...
            <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 12 }}>
              ...
              <tbody>
                {pharmacies.map(function (ph, phi) {
                  var rs = products.length + 1; var bg = phi % 2 === 0 ? '#fff' : '#f8fafc'; return <Fragment key={ph.id}>
                    {products.map(function (prd, pri) {
                      var t = pt(ph.id, prd.name); return <tr key={ph.id + '-' + prd.id} style={{ background: bg }}>
...
// ═══════════ MODALE CLÔTURE CONTRAT (Superviseur) ═══════════
function CloseContractModal({ partnership, onCloseContract }) {
  ...
  async function handleSubmit() {
    if (!status) return;
    if (status === 'Autre' && !comment.trim()) return;
    setLoading(true);
    try {
      await onCloseContract(status, comment);
      setOpen(false);
    } catch (e) { console.error(e); }
  ...
  return (
    <div>
      <button onClick={function () { setOpen(true); }}
        style={{ marginTop: 8, padding: '10px 20px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: '#dc2626', color: '#fff', border: 'none', cursor: 'pointer', width: '100%' }}>
        📋 Clôturer le contrat
      </button>

      {open && (
        <div className="backdrop" onClick={function () { setOpen(false); }}>
          <div className="bg-white rounded-xl p-6 shadow-xl" style={{ maxWidth: 480, width: '90%' }} onClick={function (e) { e.stopPropagation(); }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: '#1e293b', marginBottom: 4 }}>Clôturer le contrat</h3>
            <p style={{ fontSize: 12, color: '#64748b', marginBottom: 16 }}>{partnership.reference}</p>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
              {['Desistement', 'NonHonnore', 'Honnore', 'Autre'].map(function (opt) {
                return (
                  <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 8, cursor: 'pointer', background: status === opt ? '#eef2ff' : '#f8fafc', border: '1px solid ' + (status === opt ? '#a5b4fc' : '#e2e8f0'), fontWeight: 500, fontSize: 13, color: '#1e293b' }}>
                    <input type="radio" name="contratStatus" value={opt} checked={status === opt} onChange={function (e) { setStatus(e.target.value); }} style={{ accentColor: '#4f46e5' }} />
                    {opt === 'Desistement' && '🚫 Désistement'}
                    {opt === 'NonHonnore' && '❌ N\'a pas honoré le contrat'}
                    {opt === 'Honnore' && '✅ A honoré le contrat'}
                    {opt === 'Autre' && '📝 Autre (commentaire requis)'}
                  </label>
                );
              })}
            </div>
            ...
            {status === 'Autre' && (
              <textarea value={comment} onChange={function (e) { setComment(e.target.value); }} placeholder="Saisissez un commentaire..." rows={3}
                style={{ width: '100%', borderRadius: 8, border: '1px solid #cbd5e1', padding: '10px 12px', fontSize: 13, outline: 'none', resize: 'vertical', marginBottom: 16, boxSizing: 'border-box', fontFamily: 'inherit' }} />
            )}

            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={function () { setOpen(false); }}
                style={{ padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 500, border: '1px solid #cbd5e1', background: '#fff', color: '#475569', cursor: 'pointer' }}>Annuler</button>
              <button onClick={handleSubmit} disabled={!status || (status === 'Autre' && !comment.trim()) || loading}
                style={{ padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: status ? '#dc2626' : '#fca5a5', color: '#fff', border: 'none', cursor: status ? 'pointer' : 'not-allowed', opacity: status ? 1 : 0.5 }}>
                {loading ? 'Clôture...' : 'Clôturer définitivement'}
              </button>
            </div>
          </div>
        </div>
