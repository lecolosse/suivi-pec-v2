...
export default function PartnershipForm({ onSubmit, onCancel, currentUser }) {
  ...
  async function selectRef(ref){
    ...
    setReference(rv);setTypeM(ref.Type||'');setManifestation(ref.Manifestation||'');setProspect(ref.Prospect||'');setSupervisor(ref.sup||'');setErrors([]);setRefSelected(true);
  ...
  var dis = refSelected ? { background: '#e2e8f0', cursor: 'not-allowed', color: '#1e293b', fontWeight: 600 } : {};
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className={S} style={{borderColor:'#e2e8f0'}}>
        ...
        <div>
          ...
          <div className="relative">
            <input type="text" value={prQ||prospect} onChange={function(e){setPrQ(e.target.value);setProspect('');}} placeholder="Rechercher..." className="w-full" disabled={refSelected} style={dis} />
            {prL&&<span className="absolute animate-spin" style={{right:12,top:10}}>⏳</span>}
            {!refSelected && <Drop items={prR} show={prD} render={function(p){return <button key={p.ID} type="button" onClick={function(){setProspect(p.NOMPRENOM+' — '+p.SPECIALITE+' · '+p.SECTEUR);setPrQ('');setPrD(false);}} style={{display:'block',width:'100%',textAlign:'left',padding:'12px 16px',background:'transparent',border:'none',borderBottom:'1px solid #f1f5f9'}}><span className="font-medium">{p.NOMPRENOM}</span><span className="text-xs text-slate-400 ml-2">{p.SPECIALITE} · {p.SECTEUR}</span></button>;}} />}
          </div>
