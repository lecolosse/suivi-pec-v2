...
export default function PartnershipDetail({ partnership, onUpdate, onDelete, onBack, userRole }) {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {/* Info grid - Clean Card Style */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {infos.map(x => (
          <div key={x.l} className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-sm hover:shadow-md transition-shadow">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5 flex items-center gap-1">{x.i} {x.l}</p>
            <p className="text-sm font-bold text-slate-800 truncate" title={x.v}>{x.v}</p>
          </div>
        ))}
      </div>
                  ...
                  return (
                    <Fragment key={ph.id}>
                      {products.map((pr, pri) => {
                        ...
                        const isFirst = pri === 0;
                        const isLast = pri === products.length - 1;
