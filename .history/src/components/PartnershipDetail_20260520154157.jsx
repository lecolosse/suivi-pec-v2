...
export default function PartnershipDetail({ partnership, onUpdate, onDelete, onBack, userRole }) {
  ...
  function prescript(productName, month) {
    ...
    if (total === 0) return 0;
    return Math.round(((total / potential) * 100) * 100) / 100;  // ×100
  }
  ...
  function moyenneMensuelle(productName) {
    if (potential === 0) return 0;
    var nonZeroMonths = months.filter(function(m) { return prescript(productName, m) > 0; });
    if (nonZeroMonths.length === 0) return 0;
    var sum = nonZeroMonths.reduce(function(s, m) { return s + prescript(productName, m); }, 0);
    return Math.round((sum / nonZeroMonths.length) * 100) / 100;
  }
  ...
  function atteinteObjectif(productName) {
    ...
    if (mm === 0) return 0;
    return Math.round((mm / boxes) * 10000) / 100; // pourcentage avec 2 décimales
  }
  ...
  function exportCSV() {
    ...
    csv += "Total Produits par Mois\nProduit"; for (var k=0;k<months.length;k++) csv += ";"+formatMonth(months[k]); csv += ";Total\n";
    ...
    csv += "Prescriptions Reelles\nProduit"; for (var k=0;k<months.length;k++) csv += ";"+formatMonth(months[k]); csv += ";Total\n";
    for (var j=0;j<products.length;j++) { csv += products[j].name; for (var k=0;k<months.length;k++) csv += ";"+prescript(products[j].name,months[k]); csv += ";"+prescriptTotal(products[j].name)+"\n"; }
    csv += "TOTAL;"; for (var k=0;k<months.length;k++) csv += ";"+prescriptMonth(months[k]); csv += ";"+prescriptGrand()+"\n";
  ...
  return (
    <div className="space-y-4">
      ...
      {months.length===0?<div style={{background:'#fefce8',border:'1px solid #fde68a',borderRadius:12,padding:20,textAlign:'center',fontWeight:600,color:'#854d0e',fontSize:13}}>⚠️ Période invalide</div>:(
        <div style={{background:'#fff',borderRadius:12,border:'1px solid #e2e8f0',overflow:'hidden'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 20px',background:'#f8fafc',borderBottom:'1px solid #e2e8f0'}}>
            <div><h3 style={{fontSize:14,fontWeight:700,color:'#1e293b'}}>📊 Tableau de Suivi</h3><p style={{fontSize:12,color:'#94a3b8',marginTop:1}}>{formatMonth(months[0])} → {formatMonth(months[months.length-1])} · {months.length} mois</p></div>
        ...
        <div style={{background:'#fff',borderRadius:12,border:'1px solid #e2e8f0',overflow:'hidden'}}>
          <div style={{padding:'14px 20px',background:'#f0fdfa',borderBottom:'1px solid #ccfbf1',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <div>
              ...
              <p style={{fontSize:11,color:'#5eead4',marginTop:2}}>Formule : si Potentiel=0 → 0 | si Total Produit/Mois=0 → 0 | sinon Total Produit/Mois / Potentiel × 100 ({potential}%)</p>
            </div>
          </div>
          <div style={{overflow:'auto',maxHeight:'35vh'}}>
            <table style={{borderCollapse:'collapse',width:'100%',fontSize:12}}>
          ...
          <div style={{padding:'10px 20px',background:'#f0fdfa',borderTop:'1px solid #ccfbf1',fontSize:11,color:'#0f766e',display:'flex',flexWrap:'wrap',gap:16}}>
            <span>Si Potentiel=0 → 0</span><span>|</span><span>Si Total Produit/Mois=0 → 0</span><span>|</span><span>Sinon → Total Produit/Mois / Potentiel × <b>100</b></span>
          </div>
        </div>
      )}
      ...
      {months.length>0 && (
        <div style={{background:'#fff',borderRadius:12,border:'1px solid #e2e8f0',overflow:'hidden'}}>
          <div style={{padding:'14px 20px',background:'#fef3c7',borderBottom:'1px solid #fde68a'}}>
            <h3 style={{fontSize:14,fontWeight:700,color:'#b45309'}}>📈 Synthèse</h3>
            <p style={{fontSize:11,color:'#d97706',marginTop:2}}>Moyenne Mensuelle et % d'atteinte des objectifs par produit</p>
          </div>
          <div style={{overflow:'auto',maxHeight:'30vh'}}>
            <table style={{borderCollapse:'collapse',width:'100%',fontSize:12}}>
              <thead>
                <tr style={{background:'#d97706'}}>
                  <th style={{position:'sticky',left:0,zIndex:10,background:'#d97706',color:'#fff',fontWeight:600,padding:'10px 12px',textAlign:'left',minWidth:150,border:'1px solid #b45309',fontSize:11}}>Produit</th>
                  <th style={{background:'#d97706',color:'#fff',fontWeight:600,padding:'10px 12px',textAlign:'center',width:100,border:'1px solid #b45309',fontSize:11}}>Boîtes</th>
                  <th style={{background:'#d97706',color:'#fff',fontWeight:600,padding:'10px 12px',textAlign:'center',width:180,border:'1px solid #b45309',fontSize:11}}>Moyenne Mensuelle</th>
                  <th style={{background:'#b45309',color:'#fff',fontWeight:700,padding:'10px 12px',textAlign:'right',width:100,border:'1px solid #b45309',fontSize:11}}>% Objectif</th>
                </tr>
              </thead>
              <tbody>
                {products.map(function(prod,pri){
                  var bg = pri%2===0?'#fff':'#f8fafc';
                  var mm = moyenneMensuelle(prod.name);
                  var ao = atteinteObjectif(prod.name);
                  var boxes = prod.boxes || 0;
                  return (
                    <tr key={'synth-'+prod.id} style={{background:bg}}>
                      <td style={{position:'sticky',left:0,zIndex:5,background:bg,padding:'10px 12px',fontWeight:600,fontSize:12,color:'#1e293b',border:'1px solid #e2e8f0'}}>{prod.name}</td>
                      <td style={{border:'1px solid #e2e8f0',padding:'8px 12px',textAlign:'center',fontWeight:600,fontSize:12,color:'#475569'}}>{boxes}</td>
                      <td style={{border:'1px solid #e2e8f0',padding:'8px 12px',textAlign:'center',fontWeight:600,fontSize:12,color:mm>0?'#b45309':'#94a3b8'}}>
                        {mm>0 ? mm.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2}) : ''}
                      </td>
                      <td style={{border:'1px solid #e2e8f0',padding:'8px 12px',textAlign:'right',fontWeight:700,fontSize:13,color:ao>=100?'#16a34a':ao>0?'#d97706':'#94a3b8',background:ao>=100?'#f0fdf4':bg}}>
                        {ao>0 ? ao.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2})+' %' : ''}
                      </td>
                    </tr>
                  );
                })}
          ...
          <div style={{padding:'10px 20px',background:'#fef3c7',borderTop:'1px solid #fde68a',fontSize:11,color:'#b45309',display:'flex',flexWrap:'wrap',gap:16}}>
            <span>Moyenne Mensuelle = MOYENNE.SI(Prescriptions≠0)</span>
            <span>|</span>
            <span>% Objectif = Moyenne / Boîtes</span>
          </div>
        </div>
      )}
