...
export default function PartnershipDetail({ partnership, onUpdate, onDelete, onBack, userRole }) {
  ...
  // Pour chaque produit, par mois : quantite_produit_mois / potentiel %
  function totalProductMonth(productName, month) {
    return pharmacies.reduce(function(s, ph) {
      return s + val(ph.id, productName, month);
    }, 0);
  }

  function prescriptionReelle(productName, month) {
    if (budget === 0) return 0;
    if (potential === 0) return 0;
    var t = totalProductMonth(productName, month);
    if (t === 0) return 0;
    return Math.round((t / potential) * 100) / 100; // avec 2 décimales
  }

  function prescriptionTotal(productName) {
    return months.reduce(function(s, m) {
      return s + prescriptionReelle(productName, m);
    }, 0);
  }

  function prescriptionGrandTotal() {
    return products.reduce(function(s, pr) {
      return s + prescriptionTotal(pr.name);
    }, 0);
  }

  function prescriptionMonthTotal(month) {
    return products.reduce(function(s, pr) {
      return s + prescriptionReelle(pr.name, month);
    }, 0);
  }
  ...
  function exportCSV() {
    ...
    for (var j = 0; j < products.length; j++) {
      csv += products[j].name;
      for (var k = 0; k < months.length; k++) csv += ";" + prescriptionReelle(products[j].name, months[k]);
      csv += ";" + prescriptionTotal(products[j].name) + "\n";
    }
    csv += "TOTAL;";
    for (var k = 0; k < months.length; k++) csv += ";" + prescriptionMonthTotal(months[k]);
    csv += ";" + prescriptionGrandTotal() + "\n";
  ...
  var tdStyle = function(c) { return { border:'1px solid #e2e8f0', padding:c||'6px 6px', textAlign:'center' }; };
  var thStyle = function(extra) { return Object.assign({ border:'1px solid rgba(99,102,241,.4)', color:'rgba(255,255,255,.9)', fontWeight:600, padding:'10px 6px', textAlign:'center', fontSize:10, whiteSpace:'nowrap', minWidth:75 }, extra || {}); };
  ...
  return (
    <div className="space-y-4">
            ...
            <table style={{borderCollapse:'collapse',width:'100%',fontSize:12}}>
              <thead>
                <tr style={{background:'#4f46e5'}}>
                  ...
                  <th style={Object.assign(thStyle(),{background:'#4338ca',color:'#fff',minWidth:50})}>Σ</th>
                </tr>
              </thead>
              <tbody>
                          ...
                          <tr key={ph.id+'-'+pr.id} style={{background:bg}}>
                            {pri===0&&(
                              <td rowSpan={rs} style={{position:'sticky',left:0,zIndex:10,background:bg,padding:'10px 10px',fontWeight:700,color:'#1e293b',verticalAlign:'top',border:'1px solid #e2e8f0',fontSize:12}}>
                                <div style={{display:'flex',alignItems:'center',gap:6}}><span>🏥</span><span style={{lineHeight:1.3}}>{ph.name}</span></div>
                              </td>
                            )}
                ...
                <tr style={{background:'#e2e8f0'}}>
                  ...
                  {months.map(function(m){return <td key={m} style={{border:'1px solid #cbd5e1',padding:'8px 6px',textAlign:'center',fontWeight:700,fontSize:11,color:'#475569'}}>{mT(m)||''}</td>;})}
      ...
      {/* ═══════════════ PRESCRIPTIONS RÉELLES ═══════════════ */}
      <div style={{background:'#fff',borderRadius:12,border:'1px solid #e2e8f0',overflow:'hidden'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 20px',background:'#f0fdfa',borderBottom:'1px solid #ccfbf1'}}>
          <div>
            <h3 style={{fontSize:14,fontWeight:700,color:'#0f766e'}}>💊 Prescriptions Réelles</h3>
            <p style={{fontSize:11,color:'#5eead4',marginTop:2}}>
              Formule : Quantité Produit Mois / Potentiel ({potential}%)
          ...
          <table style={{borderCollapse:'collapse',width:'100%',fontSize:12}}>
            <thead>
              <tr style={{background:'#0d9488'}}>
                <th style={{position:'sticky',left:0,zIndex:10,background:'#0d9488',color:'#fff',fontWeight:600,padding:'10px 12px',textAlign:'left',whiteSpace:'nowrap',minWidth:120,border:'1px solid #0f766e',fontSize:11}}>Produit</th>
                {months.map(function(m){return <th key={'pr-'+m} style={{background:'#0d9488',color:'#fff',fontWeight:600,padding:'10px 6px',textAlign:'center',whiteSpace:'nowrap',minWidth:75,border:'1px solid #0f766e',fontSize:10}}>{formatMonth(m)}</th>;})}
            ...
            <tbody>
              {products.map(function(pr,pri){
                ...
                return (
                  <tr key={'pr-'+pr.id} style={{background:bg}}>
                    <td style={{position:'sticky',left:0,zIndex:5,background:bg,padding:'10px 12px',fontWeight:600,fontSize:12,color:'#1e293b',border:'1px solid #e2e8f0'}}>
                      {pr.name}
                    </td>
                    {months.map(function(month){
                      var prVal=prescriptionReelle(pr.name,month);
                      return (
                        <td key={'pr-'+pr.id+'-'+month} style={{border:'1px solid #e2e8f0',padding:'8px 6px',textAlign:'center',background:bg,fontWeight:500,fontSize:12,color:prVal>0?'#0f766e':'#94a3b8'}}>
                          {prVal>0?prVal.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2}):''}
                        </td>
                      );
                    })}
                    <td style={{border:'1px solid #e2e8f0',padding:'8px 6px',textAlign:'center',fontWeight:700,fontSize:12,color:'#0f766e',background:'#f0fdfa'}}>
                      {prescriptionTotal(pr.name)>0?prescriptionTotal(pr.name).toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2}):''}
                    </td>
                  </tr>
                );
              })}
              {/* Total Prescriptions par mois */}
              <tr style={{background:'#ccfbf1'}}>
                <td style={{position:'sticky',left:0,zIndex:5,background:'#ccfbf1',padding:'8px 12px',fontWeight:700,fontSize:11,color:'#0f766e',border:'1px solid #99f6e4'}}>📅 Total par mois</td>
                {months.map(function(m){return <td key={'pr-tm-'+m} style={{border:'1px solid #99f6e4',padding:'8px 6px',textAlign:'center',fontWeight:700,fontSize:11,color:'#0f766e'}}>{prescriptionMonthTotal(m)>0?prescriptionMonthTotal(m).toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2}):''}</td>;})}
                <td style={{border:'1px solid #99f6e4',padding:'8px 6px',textAlign:'center',fontWeight:800,fontSize:11,color:'#0f766e',background:'#99f6e4'}}>{prescriptionGrandTotal()>0?prescriptionGrandTotal().toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2}):''}</td>
              </tr>
              {/* Grand Total Prescriptions */}
              <tr style={{background:'#0d9488'}}>
                ...
                {months.map(function(m){return <td key={'pr-gt-'+m} style={{border:'1px solid #0f766e',color:'#fff',fontWeight:800,fontSize:12,padding:'10px 6px',textAlign:'center'}}>{prescriptionMonthTotal(m)>0?prescriptionMonthTotal(m).toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2}):''}</td>;})}
                <td style={{background:'#0f766e',color:'#fff',fontWeight:800,fontSize:13,padding:'10px 8px',textAlign:'center',border:'1px solid #0f766e'}}>{prescriptionGrandTotal()>0?prescriptionGrandTotal().toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2}):''}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div style={{display:'flex',flexWrap:'wrap',gap:12,padding:'10px 20px',background:'#f0fdfa',borderTop:'1px solid #ccfbf1',fontSize:11,color:'#0f766e'}}>
          <span>Formule : Quantité / Potentiel</span>
