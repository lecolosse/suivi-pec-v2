...
export default function PartnershipForm({ onSubmit, onCancel, currentUser }) {
  ...
  async function selectRef(ref){
    ...
    try { var ck = await api.checkReference(rv); if(ck.exists){ setErrors(['⚠️ La référence "'+rv+'" est déjà utilisée !']); return; } } catch(e){}
    setReference(rv);setTypeM(ref.Type||'');setManifestation(ref.Manifestation||'');setProspect(ref.Prospect||'');setSupervisor(ref.sup||'');setRefD(false);setRefQ('');setErrors([]);
