...
export default function PartnershipDetail({ partnership, onUpdate, onDelete, onBack, userRole }) {
  ...
  async function configMonth(month, status, mc) {
    try {
      ...
      var res = await fetch('http://localhost:3001/api/partenariats/' + partnership.id + '/month-config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify({ month: month, status: status, manualClose: mc })
      });
            ...
            {months.map(function(m) {
              ...
              return (
                <div key={'cfg-' + m} style={{
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: isClosed ? 'not-allowed' : 'pointer', flex: 1, opacity: isClosed ? 0.5 : 1 }}>
                    <input
                      type="radio"
                      name="activeMonth"
                      checked={isActive}
                      disabled={isClosed}
                      onChange={function() { if (!isClosed) configMonth(m, 'active', mc); }}
                      style={{ accentColor: '#16a34a', width: 18, height: 18, cursor: isClosed ? 'not-allowed' : 'pointer' }}
                    />
