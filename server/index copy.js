...
function buildPartnership(p, prods, pharmas, qtys, creatorName) {
  const tableData = {};
  for (const q of qtys) {
...
async function getCreatorName(createdBy) {
  if (!createdBy) return '';
  try {
    const [rows] = await db.query('SELECT login FROM users WHERE ID = ?', [createdBy]);
    return rows.length > 0 ? rows[0].login : '';
  } catch { return ''; }
}
...
app.get('/api/partenariats', authMiddleware, async (req, res) => {
  try {
    ...
    for (const p of rows) {
      ...
      const creatorName = await getCreatorName(p.created_by);
      result.push(buildPartnership(p, prods, pharmas, qtys, creatorName));
    }
...
app.get('/api/partenariats/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const { prods, pharmas, qtys } = await loadRelated(p.id);
    const creatorName = await getCreatorName(p.created_by);
    res.json({ success: true, data: buildPartnership(p, prods, pharmas, qtys, creatorName) });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
