...
export var api = {
  ...
  closeContract: function(id, status, comment) { return apiCall('/partenariats/' + id + '/close-contract', { method: 'POST', body: JSON.stringify({ status: status, comment: comment || '' }) }); },
  checkReference: function(ref) { return apiCall('/partenariats/check-reference?ref=' + encodeURIComponent(ref)); },
};
